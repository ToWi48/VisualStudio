...
#End Region
        ...
        Try
            If Me.Owner.GetType Is GetType(frmMain) Then
                ...
                If txtFTPRoot.Text = "" Then
                    If Not frm.fpath = "" Then
                        LoadDialog(Path.GetDirectoryName(frm.fpath), Path.GetFileName(frm.fpath), False)
                    Else
                        LoadDialog(frm.ApplicationDataFolder(True, "") & "\", "", False)
                    End If
                Else
                    ListDirectoryDetails()
                End If
                If Not Path.GetFileName(frm.fpath) = "" Then
                    SelectFileName(Path.GetFileName(frm.fpath))
                End If
            End If
        Catch ex As Exception
            Err.Clear()
    ...
    Public Class FileSystemItemVB
        ...
        Private _PageSelection As String = ""
        ...
        Public Property FileName() As String
            ...
            End Set
        End Property
        <System.ComponentModel.DisplayName("FullPath")>
        ...
        Private Function IsMatch(ByVal extension As String, ByVal ParamArray extensionsToCheck As String()) As Boolean
            ...
            Return False
        End Function
        <System.ComponentModel.DisplayName("Pages")>
        Public Property PageSelection() As String
            Get
    ...
    Private Sub DataGridView1_ColumnHeaderMouseClick(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellMouseEventArgs) Handles DataGridView1.ColumnHeaderMouseClick
        ...
        End Try
    End Sub
    Public listSelectedPaths As New Dictionary(Of String, String)
        ...
        Try
            ...
            If e.Button = Windows.Forms.MouseButtons.Right Then
                ...
                If rowIndex >= 0 Then
                    ...
                    End If
                End If
            ElseIf e.Button = Windows.Forms.MouseButtons.Left And Not blnCellClick Then
                ...
                If hittestInfo1.RowIndex > 0 Then
                    ...
                    If hittestInfo1.ColumnIndex = 0 Then
                            ...
                            If Not DataGridView1.Rows(hittestInfo1.RowIndex).Cells("FileName").Value.ToString.ToLower.Contains("▲ Up".ToLower) Then
                                ...
                                If Directory.Exists(fpth) Then
                                    If CBool(DirectCast(DataGridView1.Rows(hittestInfo1.RowIndex).Cells("chkSelected"), DataGridViewCheckBoxCell).Value) = True Then
                                        If Not listSelectedPaths.Keys.Contains(fpth) Then
                                            Try
                                                For Each lineDir As String In Directory.GetFiles(fpth)
                                                    Try
                                                        If Not listSelectedPaths.Keys.Contains(lineDir) Then
                                                            listSelectedPaths.Add(lineDir, DataGridView1.Rows(hittestInfo1.RowIndex).Cells("chkSelected").Value)
                                                        End If
                                                    Catch exReadLines As Exception
                                                        If frm.debugMode Then Throw exReadLines Else Err.Clear()
                                                    End Try
                                    ...
                                    Else
                                        Try
                                            For Each lineDir As String In Directory.GetFiles(fpth)
                                                Try
                                                    If listSelectedPaths.Keys.Contains(lineDir) Then
                                ...
                                ElseIf File.Exists(fpth) Then
                                    If CBool(DirectCast(DataGridView1.Rows(hittestInfo1.RowIndex).Cells("chkSelected"), DataGridViewCheckBoxCell).Value) = True Then
                                        If Not listSelectedPaths.Keys.Contains(fpth) Then
                                            listSelectedPaths.Add(fpth, DataGridView1.Rows(hittestInfo1.RowIndex).Cells("PageSelection").Value)
                                        End If
                                    Else
                                        If listSelectedPaths.Keys.Contains(fpth) Then
                        ...
                        End If
                    End If
                ElseIf hittestInfo1.ColumnIndex = 0 And Not blnCellClick Then
                    DataGridView1.Rows(hittestInfo1.RowIndex).Selected = True
                    If hittestInfo1.ColumnIndex = 0 Then
                        If DataGridView1.Rows(hittestInfo1.RowIndex).Cells("chkSelected").GetType Is GetType(DataGridViewCheckBoxCell) Then
                            If CBool(DirectCast(DataGridView1.Rows(hittestInfo1.RowIndex).Cells("chkSelected"), DataGridViewCheckBoxCell).Value) = False Then
    ...
    Public Sub DataGridView1_SelectionChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles DataGridView1.SelectionChanged
        ...
        Try
            ...
            Else
                GroupBoxFolder1.Visible = False
                GroupBoxFile1.Visible = True
                txtFileName.Text = fsItems(DataGridView1.SelectedRows(0).Index).FullPath
                Dim fileBytes1() As Byte = Nothing
                Try
                    fileBytes1 = File.ReadAllBytes(fsItems(DataGridView1.SelectedRows(0).Index).FullPath)
                    lblUpload_FileName.Text = "File Name: " & Environment.NewLine & "Size: " & frm.getMegaBytesText(fileBytes1.Length) & " "
                Catch ex As Exception
                ...
                Select Case Path.GetExtension(txtFileName.Text & "").ToString.ToLower.TrimStart("."c)
                    Case "pdf"
                        ...
                        Else
                            btnFileLoadPdfFileIntoPdfEditor.Visible = False
                        End If
                        txtPageSelection.Text = CStr(DataGridView1.SelectedRows(0).Cells("PageSelection").Value)
...
GOTO_DOWNLOAD:
                    DownloadFile(CStr(fsItems(DataGridView1.SelectedRows(0).Index).FullPath), sfd.SelectedPath.ToString.TrimEnd("\"c) & "\"c & CStr(DataGridView1.SelectedRows(0).Cells("FileName").Value), openFolder, openDefaultViewer, openPdfEditor, False)
                    If openPdfEditor Then
                        frm.dlgValues.Clear()
    ...
    Private Sub Button2_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button2.Click
        Try
            ListFileDetails(listSelectedPaths.Keys.ToList())
        Catch ex As Exception
            Err.Clear()
        End Try
    End Sub
    ...
    End Sub
    Private Sub txtPageSelection_TextChanged(sender As Object, e As EventArgs) Handles txtPageSelection.TextChanged
        Try
            If Not DataGridView1.SelectedRows(0) Is Nothing Then
                DataGridView1.SelectedRows(0).Cells("PageSelection").Value = CStr(txtPageSelection.Text)
                If listSelectedPaths.Keys.Contains(fsItems(DataGridView1.SelectedRows(0).Index).FullPath) Then
                    listSelectedPaths(fsItems(DataGridView1.SelectedRows(0).Index).FullPath) = CStr(txtPageSelection.Text)
                End If
            End If
        Catch ex As Exception
            Err.Clear()
    ...
    Private Sub moveUp()
        If DataGridView1.RowCount > 0 Then
            If DataGridView1.SelectedRows.Count > 0 Then
                ...
                Dim index As Integer = DataGridView1.SelectedCells(0).OwningRow.Index
                If index = 0 Then
                    Return
                End If
    ...
    Private Sub moveDown()
        ...
        End If
    End Sub
    Public blnCellClick As Boolean = False
    Private Sub DataGridView1_CellClick(sender As Object, e As DataGridViewCellEventArgs) Handles DataGridView1.CellClick
        Try
            Dim idx As Integer = e.RowIndex
            If Not DataGridView1.Rows(idx).Cells("FileType").Value = "File folder" Then
                If e.ColumnIndex = 7 Then
                    blnCellClick = True
                    fsItems = DataGridView1.DataSource
                    '
                    '
                    '
                    If Not DataGridView1.Rows(idx - 1).Cells("FileType").Value = "File folder" Then
                        Dim chk As New DataGridViewCheckBoxCell
                        chk = DirectCast(DataGridView1.Rows(idx).Cells("chkSelected"), DataGridViewCheckBoxCell)
                        Dim r As FileSystemItemVB = fsItems(idx)
                        fsItems.RemoveAt(idx)
                        fsItems.Insert(idx - 1, r)
                        DataGridView1.ClearSelection()
                        DataGridView1.DataSource = fsItems
                        DataGridView1.Focus()
                        DataGridView1.Select()
                        Dim newIdx As Integer = idx - 1
                        DataGridView1.FirstDisplayedScrollingRowIndex = newIdx
                        DataGridView1.CurrentCell = DataGridView1.Rows(newIdx).Cells(0)
                        DataGridView1.Rows(newIdx).Selected = True
                        DataGridView1.Rows(newIdx).Cells(0).Selected = True
                        DataGridView1.Refresh()
                        DirectCast(DataGridView1.Rows(idx - 1).Cells("chkSelected"), DataGridViewCheckBoxCell).Value = chk.Value
                    End If
                    '
