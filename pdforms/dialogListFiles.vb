...
GOTO_DOWNLOAD:
    ...
    Private Sub btnExportSelected_Click(sender As Object, e As EventArgs) Handles btnExportSelected.Click
        ...
        Dim dt As New DataTable("pdforms")
        Try
            Dim dc As DataColumn
            Dim dr As DataRow
            For r As Integer = 0 To listSelectedPaths.Count - 1
                Try
                    If System.IO.File.Exists(listSelectedPaths(r)) Then
                        Dim cfdf As New FDFApp.FDFDoc_Class
                        Dim cpdf As New FDFApp.FDFApp_Class
                        Dim b() As Byte = Nothing
                        Try
                            b = System.IO.File.ReadAllBytes(listSelectedPaths(r))
                            If Not frm.IsPasswordProtected(b) Then
                                Try
                                    cfdf = cpdf.PDFOpenFromBuf(b)
                                    For Each fld As FDFApp.FDFDoc_Class.FDFField In cfdf.XDPGetAllFields(True)
                                        If Not fldNames.Contains(fld.FieldName) And Not String.IsNullOrEmpty(fld.FieldName) Then
                                            fldNames.Add(fld.FieldName)
                                            dc = New DataColumn(fld.FieldName, GetType(System.String))
                                            dc.MaxLength = 9999
                                    ...
                                    Next
                                    dr = dt.NewRow
                                    dr = cfdf.XDPSetDataRowFromValues(dr)
                                    dt.Rows.Add(dr)
                                Catch ex As Exception
                                    Err.Clear()
                                End Try
                            ElseIf frm.IsPasswordProtected(b) Then
                                Try
                                    b = frm.UnlockSecurePDF(b)
                                    cfdf = cpdf.PDFOpenFromBuf(b)
                                    For Each fld As FDFApp.FDFDoc_Class.FDFField In cfdf.XDPGetAllFields(True)
                                        If Not fldNames.Contains(fld.FieldName) And Not String.IsNullOrEmpty(fld.FieldName) Then
                                            fldNames.Add(fld.FieldName)
        ...
        Finally
            If Not dt Is Nothing Then
                If dt.Rows.Count > 0 Then
                    Dim SaveAsFileDialog1 As New frmSaveAs()
                    SaveAsFileDialog1.o.Filter = "XML|*.xml|XLS|*.xls|Json|*.json"
                    SaveAsFileDialog1.o.FilterIndex = 0
                    SaveAsFileDialog1.o.AutoUpgradeEnabled = True
                    SaveAsFileDialog1.o.DefaultExt = ".xml"
                    SaveAsFileDialog1.o.FilterIndex = 1
                    SaveAsFileDialog1.o.Title = "Save As"
                    SaveAsFileDialog1.o.ValidateNames = True
                    SaveAsFileDialog1.o.OverwritePrompt = True
                    SaveAsFileDialog1.ShowFrmSaveAs_PDFVersion(False)
                    Try
                            ...
                            Case Windows.Forms.DialogResult.OK, Windows.Forms.DialogResult.Yes
                                ...
                                If Not String.IsNullOrEmpty(SaveAsFileDialog1.frmSaveAs_TextFilePath.Text) Then
                                    ...
                                    Select Case frm.GetFileExtension(fn).ToString.Replace(".", "").ToLower
                                        Case "xml"
                                            '
                                            dt.WriteXml(fn, XmlWriteMode.IgnoreSchema)
                                            frm.StatusToolStrip = "Status: File exported successfully."
                                            If MsgBox("Do you want to open the output folder?", MsgBoxStyle.YesNo Or MsgBoxStyle.ApplicationModal, "Export Success") = MsgBoxResult.Yes Then
                                                Process.Start(System.IO.Path.GetDirectoryName(fn).ToString.TrimEnd("\"c) & "\")
                                            End If
                                        Case "xls"
                                            dt.WriteXml(fn, XmlWriteMode.IgnoreSchema)
                                            frm.StatusToolStrip = "Status: File exported successfully."
                                        ...
                                        Case "json"
                                            Dim m As New MemoryStream
                                            dt.WriteXml(m, XmlWriteMode.IgnoreSchema)
                                            Dim cfdf As New FDFApp.FDFDoc_Class
                                            Dim cpdf As New FDFApp.FDFApp_Class
                                            Dim strXML As String = System.Text.Encoding.UTF8.GetString(m.ToArray)
                                            Dim doc As New System.Xml.XmlDocument
                                            doc.LoadXml(strXML)
                                            System.IO.File.WriteAllText(fn, Newtonsoft.Json.JsonConvert.SerializeXmlNode(doc))
                                            frm.StatusToolStrip = "Status: File exported successfully."
                                            If MsgBox("Do you want to open the output folder?", MsgBoxStyle.YesNo Or MsgBoxStyle.ApplicationModal, "Export Success") = MsgBoxResult.Yes Then
                                                Process.Start(System.IO.Path.GetDirectoryName(fn).ToString.TrimEnd("\"c) & "\")
                                            End If
                        ...
                        End Select
                    Catch ex As Exception
                        frm.StatusToolStrip = "Error: File not exported."
