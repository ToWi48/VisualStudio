...
#Region "  Hash"
    ...
    Public Class Hash
        ...
        Private _HashValue As New Encryption.clsData
        Private Sub New()
        End Sub
        ''' <summary>
        ...
        ''' </summary>
        Public ReadOnly Property Value() As Encryption.clsData
            Get
                Return _HashValue
            End Get
        End Property
        ...
        ''' </summary>
        Public Function Calculate(ByRef s As System.IO.Stream) As Encryption.clsData
        ...
        ''' </summary>
        Public Function Calculate(ByVal d As Encryption.clsData) As Encryption.clsData
        ...
        ''' </summary>
        Public Function Calculate(ByVal d As Encryption.clsData, ByVal salt As Encryption.clsData) As Encryption.clsData
        ...
        ''' </summary>
        Private Function CalculatePrivate(ByVal b() As Byte) As Encryption.clsData
...
#Region "  Symmetric"
    ...
    Public Class Symmetric
        Protected Function _DefaultIntializationVector() As String
            Return System.Text.Encoding.UTF8.GetString(System.Convert.FromBase64String("ZkBOYThMIVMk")).ToString & ""
        End Function
        Protected Const _BufferSize As Integer = 2048
        Public Enum Provider
            ''' <summary>
            ...
            TripleDES
        End Enum
        Private _data As Encryption.clsData
        Private _key As Encryption.clsData
        Private _iv As Encryption.clsData
        ...
        Public Sub New(ByVal provider As Provider, Optional ByVal useDefaultInitializationVector As Boolean = True)
            ...
            End Select
            _crypto.Padding = PaddingMode.None
            Me.Key = RandomKey()
            If useDefaultInitializationVector Then
                Me.IntializationVector = New Encryption.clsData(_DefaultIntializationVector)
            Else
        ...
        ''' </summary>
        Public Property Key() As Encryption.clsData
            Get
                Return _key
            End Get
            Set(ByVal Value As Encryption.clsData)
                _key = Value
        ...
        ''' </summary>
        Public Property IntializationVector() As Encryption.clsData
            Get
                Return _iv
            End Get
            Set(ByVal Value As Encryption.clsData)
        ...
        ''' </summary>
        Public Function RandomInitializationVector() As Encryption.clsData
            _crypto.GenerateIV()
            Dim d As New Encryption.clsData(_crypto.IV)
            Return d
        End Function
        ''' <summary>
        ''' generates a random Key, if one was not provided
        ''' </summary>
        Public Function RandomKey() As Encryption.clsData
            _crypto.GenerateKey()
            Dim d As New Encryption.clsData(_crypto.Key)
            Return d
        End Function
        ''' <summary>
        ...
        ''' </summary>
        Public Function Encrypt(ByVal d As Encryption.clsData, ByVal key As Encryption.clsData) As Encryption.clsData
            Me.Key = key
            Return Encrypt(d)
        End Function
        ''' <summary>
        ...
        ''' </summary>
        Public Function Encrypt(ByVal d As Encryption.clsData) As Encryption.clsData
            ...
            cs.Close()
            ms.Close()
            Return New Encryption.clsData(ms.ToArray)
        End Function
        ''' <summary>
        ''' Encrypts the System.IO.Stream to memory using provided key and provided initialization vector
        ''' </summary>
        Public Function Encrypt(ByVal s As System.IO.Stream, ByVal key As Encryption.clsData, ByVal iv As Encryption.clsData) As Encryption.clsData
        ...
        ''' </summary>
        Public Function Encrypt(ByVal s As System.IO.Stream, ByVal key As Encryption.clsData) As Encryption.clsData
            Me.Key = key
            Return Encrypt(s)
        End Function
        ''' <summary>
        ...
        ''' </summary>
        Public Function Encrypt(ByVal s As System.IO.Stream) As Encryption.clsData
            Dim ms As New IO.MemoryStream
        ...
        ''' </summary>
        Public Function Decrypt(ByVal encryptedData As Encryption.clsData, ByVal key As Encryption.clsData) As Encryption.clsData
            Me.Key = key
        ...
        ''' </summary>
        Public Function Decrypt(ByVal encryptedStream As System.IO.Stream, ByVal key As Encryption.clsData) As Encryption.clsData
            Me.Key = key
        ...
        ''' </summary>
        Public Function Decrypt(ByVal encryptedStream As System.IO.Stream) As Encryption.clsData
        ...
        ''' </summary>
        Public Function Decrypt(ByVal encryptedData As Encryption.clsData) As Encryption.clsData
            ...
            Finally
                cs.Close()
            End Try
            Return New Encryption.clsData(b)
        End Function
    End Class
#End Region
#Region "QUICKRYPT"
    Friend Class QUICKRYPT
        ...
        End Function
        Public Shared Function QuickEncrypt(ByVal value As String, Optional ByVal key As String = "l7p3zhg1g6iqui91", Optional ByVal urlFriendly As Boolean = True) As String
            Dim nkrypted As New Encryption.clsData((value), System.Text.Encoding.UTF8), dkrypt As New Encryption.Symmetric(Encryption.Symmetric.Provider.Rijndael, True)
            dkrypt.Key = New Encryption.clsData(key, System.Text.Encoding.UTF8)
            Dim strDKryptedData As String = Convert.ToBase64String(dkrypt.Encrypt(nkrypted).Bytes)
            If urlFriendly Then
                Return (strDKryptedData).ToString.Replace("/", "-").Replace("==", "@").Replace("+", "!")
            Else
                Return strDKryptedData
            End If
        End Function
        Public Shared Function QuickDecrypt(ByVal value As String, Optional ByVal key As String = "l7p3zhg1g6iqui91", Optional ByVal urlFriendly As Boolean = True) As String
            Dim filePath As String = ""
            Try
                Dim nkrypted As New Encryption.clsData, dkrypt As New Encryption.Symmetric(Symmetric.Provider.Rijndael, True)
                nkrypted.Encoding = System.Text.Encoding.UTF8
                dkrypt.Key = New Encryption.clsData(key, System.Text.Encoding.UTF8)
                If urlFriendly Then
                    nkrypted.Base64 = (value).Replace("-", "/").Replace("@", "==").Replace("!", "+")
                Else
                    nkrypted.Base64 = (value)
                End If
                filePath = (dkrypt.Decrypt(nkrypted).ToString())
            Catch ex As Exception
                filePath = ""
                Err.Clear()
            End Try
...
#Region "  PrivateKey Class"
        ...
        Public Class PrivateKey
            ...
            Public Sub ExportToConfigFile(ByVal strFilePath As String)
                Dim sw As New System.IO.StreamWriter(strFilePath, False)
                sw.Write(Me.ToConfigSection)
                sw.Close()
            End Sub
            ''' <summary>
...
#End Region
        ...
        ''' </summary>
        Public Function Encrypt(ByVal d As Encryption.clsData, ByVal publicKey As PublicKey) As Encryption.clsData
        ...
        ''' </summary>
        Public Function Encrypt(ByVal d As Encryption.clsData, ByVal publicKeyXML As String) As Encryption.clsData
            LoadKeyXml(publicKeyXML, False)
            Return EncryptPrivate(d)
        End Function
        Private Function EncryptPrivate(ByVal d As Encryption.clsData) As Encryption.clsData
            Try
                Return New Encryption.clsData(_rsa.Encrypt(d.Bytes, False))
            Catch ex As CryptographicException
        ...
        ''' </summary>
        Public Function Decrypt(ByVal encryptedData As Encryption.clsData, ByVal PrivateKey As PrivateKey) As Encryption.clsData
        ...
        ''' </summary>
        Public Function Decrypt(ByVal encryptedData As Encryption.clsData, ByVal PrivateKeyXML As String) As Encryption.clsData
        ...
        Private Sub LoadKeyXml(ByVal keyXml As String, ByVal isPrivate As Boolean)
           ...
           String.Format("The provided {0} encryption key XML does not appear to be valid.", s), ex)
            End Try
        End Sub
        Private Function DecryptPrivate(ByVal encryptedData As Encryption.clsData) As Encryption.clsData
            Return New Encryption.clsData(_rsa.Decrypt(encryptedData.Bytes, False))
        End Function
        ''' <summary>
