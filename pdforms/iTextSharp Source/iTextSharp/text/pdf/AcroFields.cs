    ...
    public class AcroFields {
        ...
        */    
        ...
        internal PdfAppearance GetAppearance(PdfDictionary merged, String[] values, String fieldName) {
            ...
            if (opt != null) {
                ...
                // EDITED NICHOLAS KOWALEWICZ
                tx.VisibleTopChoice = 0;
            }
        ...
        */
        virtual public void MergeXfaData(XmlNode n) {
            ...
            foreach (String name in data.Order)
            {
                ...
                XmlNode parentNode = (XmlNode)data.Name2Node[name].ParentNode;
                string strNamespaceURI = parentNode.NamespaceURI;
                string parentNodeFullName = name;
                if (name.LastIndexOf(".") > 0) {
                    parentNodeFullName = name.Substring(0, name.LastIndexOf("."));
                }
                parentNodeFullName = parentNodeFullName.Replace("xdp[0].datasets[0].data[0].", "");
                if (GetFieldType(parentNodeFullName).Equals(AcroFields.FIELD_TYPE_LIST)){
                    int ff = GetFieldItem(parentNodeFullName).GetMerged(0).GetAsNumber(PdfName.FF).IntValue;
                    int ffTrue = (ff & BaseField.MULTISELECT);
                    ...
                    if ( ffTrue != 0)
                    {
                        ...
                        int idx = -1;
                        String[] vals;
                        vals = new String[parentNode.ChildNodes.Count];
                        //PdfArray valsArray = new PdfArray();
                        foreach (XmlNode childNodes in parentNode.ChildNodes)
                        {
                            string childNodeName = childNodes.Name;
                            string tempMe = childNodeName;
                            idx = idx + 1;
                            vals[idx] = childNodes.InnerText;
                            //valsArray.Add(new PdfString(childNodes.InnerText));
                            //vals.Add(childNodes.InnerText);

                        }
                        if (vals.Length > 0)
                        {
                            String[] valsSet = GetListSelection(parentNodeFullName);
                            if (IsSequenceEqual(valsSet,vals)==false)
                            {
                                SetListSelection(parentNodeFullName, vals);
                                //GetFieldItem(parentNodeFullName).GetMerged(0).Put(PdfName.V, valsArray);
                            }
                        }
                    }
                    else {
                ...
                //text = XfaForm.GetNodeText((XmlNode)data.Name2Node[name]);
                //text = text;
            }
        }
        ...
        */
        virtual public bool SetField(String name, String value, String display, bool saveAppearance) {
            ...
            System.Collections.Generic.List<string> vals = new System.Collections.Generic.List<string>();
            if (xfa.XfaPresent) {
                string nameTemp = name;//topmostSubform[0].LISTBOX[0].value[0]
                name = xfa.FindFieldName(name, this);
                if (name == null)
                    return false;
                ...
                if (nameTemp.Contains("value") && !name.Contains("value"))
                {
                    if (nameTemp.ToString().Split(".".ToCharArray()).Length > 0)
                    {
                        string[] namesSplit = nameTemp.ToString().Split(".".ToCharArray());
                        if (namesSplit[namesSplit.Length - 1].ToString().StartsWith("value"))
                        {
                            if (xn.InnerXml.Contains("<value>"))
                            {
                                string[] valStringTemp = new string[] { "<value>".ToString() };
                                string[] valStrings = new string[xn.InnerText.ToString().Split(valStringTemp, StringSplitOptions.RemoveEmptyEntries).Length-1];
                                valStrings = xn.InnerText.ToString().Split(valStringTemp, StringSplitOptions.RemoveEmptyEntries);
                                for (int valIndex = 0; valIndex < valStrings.Length; valIndex++)
                                {
                                    string valString = valStrings[valIndex].ToString().Replace("<value>","").Replace("</value>","");
                                    vals.Add(valString);
                                }
                                xn.InnerXml = xn.InnerXml.ToString() + "<value>" + value + "</value>";
                                vals.Add(value);

                                //xfa.SetNodeText(xn, xn.InnerXml.ToString() + "<value>" + value + "</value>");
                            }
                            else {
                                vals.Add(value);
                                xn.InnerXml = "<value>" + value + "</value>";
                                //xfa.SetNodeText(xn, "<value>" + value + "</value>");
                            }
                        }
                    }
            ...
            if (PdfName.TX.Equals(type) || PdfName.CH.Equals(type)) {
                ...
                for (int idx = 0; idx < item.Size; ++idx)
                    {
                    PdfArray valArray = new PdfArray();
                    ...
                    if (PdfName.CH.Equals(type))
                    {
                        ...
                        if (vals.Count > 1)
                        {
                            for (int valIndex = 0; valIndex < vals.Count; valIndex++)
                            {
                                valArray.Add(new PdfString(vals[valIndex]));
                            }
                            //valueDic.Put(PdfName.V, valArray);
                            //generateAppearances = true;
                            //valueDic.Remove(PdfName.DV);
                        ...
                        if (vals.Count > 0)
                        {
                            ...
                            foreach (PdfArray opt in merged.GetAsArray(PdfName.OPT))
                            {
                                optIndex++;
                                //foreach (PdfArray opt in opts.GetAsArray(optIndex))
                                {
                                    if (opt.Size == 2)
                                    {
                                        if (vals.Contains(opt.GetAsString(1).ToUnicodeString().TrimStart("/".ToCharArray())))
                                        {
                                            indexArray.Add(new PdfNumber(optIndex));
                                        }
                                    }
                                    else
                                    {
                                        if (vals.Contains(opt.GetAsString(0).ToUnicodeString().TrimStart("/".ToCharArray())))
                                        {
                                            indexArray.Add(new PdfNumber(optIndex));
                                        }
                                    }
                        ...
                        if (vals.Count > 1 | (vals.Count > 0 && optIndex > 1))
                        {
                            //valueDic.Put(PdfName.V, valArray);
                            //MarkUsed(valueDic);
                            this.SetListSelection(name, vals.ToArray());
                            //merged = item.GetMerged(idx);
                            return true;
                            //valueDic.Put(PdfName.DV, valArray);

                            //MarkUsed(merged);
                            //merged.Remove(PdfName.DV);
                            //MarkUsed(merged);
                            //generateAppearances = true;
                            //valueDic.Remove(PdfName.DV);
                        }
                        else
                        {
                            ...
                            merged.Put(PdfName.V, v);
                            //valueDic.Put(PdfName.V, v);
                            //valueDic.Put(PdfName.DV, v);
                            //MarkUsed(merged);
                            //SetListSelection(name, vals.ToArray());
                            //return true;
                            //merged.Remove(PdfName.DV);
                            //MarkUsed(merged);
                            MarkUsed(valueDic);
                        ...
                        {
                            merged.Put(PdfName.I, indexArray);
                        }
                        else if (indexArray.Size > 0)
                        {
                            merged.Put(PdfName.I, indexArray[0]);
                        }
                    }
                    else
                    {
                    ...
                    //    item.GetMerged(idx).Put(PdfName.DV, valArray);
                    //    item.GetWidget(idx).Put(PdfName.DV, valArray);
                    //    item.GetMerged(idx).Put(PdfName.V, valArray);
                    //    item.GetWidget(idx).Put(PdfName.V, valArray);
                    //    //widget.Put(PdfName.DV, valArray);
                    //}
                    //else
                    //{
                    //    //widget.Put(PdfName.DV, v);
                    //    item.GetMerged(idx).Put(PdfName.DV, v);
                    //    item.GetWidget(idx).Put(PdfName.DV, v);
                    //    item.GetMerged(idx).Put(PdfName.V, v);
                    //    item.GetWidget(idx).Put(PdfName.V, v);
                    //}
                    //PdfDictionary widget2 = item.GetWidget(idx);
                    //if (generateAppearances) {
                    //    PdfAppearance app = GetAppearance(merged, display, name);
                    //    if (PdfName.CH.Equals(type)) {
                    //        PdfNumber n = new PdfNumber(topFirst);
                    //        widget2.Put(PdfName.TI, n);
                    //        merged.Put(PdfName.TI, n);
                    //    }
                    //    PdfDictionary appDic = widget2.GetAsDict(PdfName.AP);
                    //    if (appDic == null) {
                    //        appDic = new PdfDictionary();
                    //        widget2.Put(PdfName.AP, appDic);
                    //        merged.Put(PdfName.AP, appDic);
                    //    }
                    //    appDic.Put(PdfName.N, app.IndirectReference);
                    //    writer.ReleaseTemplate(app);
        ...
        */
        virtual public bool SetListSelection(String name, String[] value) {
            ...
            //String[] options = GetListOptionExport(name);
            String[] options = GetListOptionDisplay(name);
            PdfArray array = new PdfArray();
            PdfArray vals = new PdfArray();
            string[] values = GetListOptionExport(name);
            System.Collections.Generic.List<string> valueSelections = new System.Collections.Generic.List<string>();
            foreach (string element in value) {
                for (int j = 0; j < options.Length; j++) {
                    if (values[j].Equals(element))
                    {
                        array.Add(new PdfNumber(j));
                        vals.Add(new PdfString(options[j]));
                        //vals.Add(new PdfString(values[j]));
                        valueSelections.Add(values[j]);
                    }
                    //else if (options[j].Equals(element))
                    //{
                    //    array.Add(new PdfNumber(j));
                    //    vals.Add(new PdfString(options[j]));
                    //}
            ...
            //widget.Remove(PdfName.I);
            //MarkUsed(item.GetWidget(0));
            ...
            //PdfAppearance app = GetAppearance(merged, valueSelections.ToArray(), name);
        ...
        */    
        virtual public Item GetFieldItem(String name) {
            ...
            if (!fields.ContainsKey(name)) {
                ...
                {
                    if (fields.ContainsKey(name.ToString().Substring(0, name.ToString().IndexOf("["))))
                    {
                        return fields[name.ToString().Substring(0, name.ToString().IndexOf("["))];
                    }
                }
                return null;
            }
        ...
        */    
        ...
        // EDITED NICHOLAS KOWALEWICZ
        virtual public bool RemoveField(String name, int page, int index)
        {
            Item item = GetFieldItem(name);
            if (item == null)
                return false;
            ...
            if (arrayf == null)
                return false;
            //for (int k = 0; k < item.Size; ++k)
            if (index >= 0 && item.Size > index)
            {
                int k = index;
                int pageV = item.GetPage(k);
                //if (page != -1 && page != pageV) { continue; }
                if (page == pageV || page == -1)
                {
                    PdfIndirectReference refi = item.GetWidgetRef(k);
                    PdfDictionary wd = item.GetWidget(k);
                    PdfDictionary pageDic = reader.GetPageN(pageV);
        ...
        */
        ...
        virtual public PushbuttonField GetNewPushbuttonFromField(String field, int order, PdfWriter writer)
        {
            if (GetFieldType(field) != FIELD_TYPE_PUSHBUTTON)
                return null;
            Item item = GetFieldItem(field);
            ...
            if (newButton.Writer == null)
            {
                if (writer != null)
                {
                    newButton.Writer = writer;
                }
            }
            return newButton;
        }
        public TextField GetNewTextFieldFromField(String fieldName, int order, PdfWriter writer)
        {
            try
            {
                if (GetFieldType(fieldName) != FIELD_TYPE_TEXT)
                    return null;
                //if (order){order=0;}
                if (order <= 0)
                {
                ...
                topFirst = 0;
                TextField tx = null;
                Item item = GetFieldItem(fieldName);
                PdfDictionary merged = (PdfDictionary)item.merged[order];
                if (fieldCache == null || !fieldCache.ContainsKey(fieldName))
                {

                    //tx = new TextField(writer, null,null);
                    IList<FieldPosition> pos = GetFieldPositions(fieldName);
                    Rectangle box = new Rectangle(pos[order].position);
                    tx = new TextField(writer, box, fieldName);
                    tx.SetExtraMargin(extraMarginLeft, extraMarginTop);
                    tx.BorderWidth = 0;
                    tx.SubstitutionFonts = substitutionFonts;
                    DecodeGenericDictionary(merged, tx);
                ...
                PdfDictionary dict = item.GetWidget(order);
                PdfDictionary mk = dict.GetAsDict(PdfName.MK);
                iTextSharp.text.BaseColor c;
                iTextSharp.text.BaseColor bordercolor = new iTextSharp.text.BaseColor(0, 0, 0);
                iTextSharp.text.BaseColor bgcolor = new iTextSharp.text.BaseColor(255, 255, 255);
                iTextSharp.text.BaseColor textcolor = new iTextSharp.text.BaseColor(0, 0, 0);
                ...
                try
                {
                    //if(da!=null) 
                    //{
                    dict = item.GetWidget(order);
                    PdfString da = dict.GetAsString(PdfName.DA);
                    if (da != null)
                    {
                        object[] dao = AcroFields.SplitDAelements(da.ToUnicodeString());
                        object[] colors = AcroFields.SplitDAelements(item.GetWidget(order).GetAsString(PdfName.DA).ToString());
                        textcolor = (iTextSharp.text.BaseColor)(colors[colors.Length - 1]);
                    };
                }
                catch (Exception ex) { }

                string textVal = GetField(fieldName);
                if (textVal != null)
                {
                    tx.Text = textVal;//.ToUnicodeString();
                }
                if (mk != null)
                {
                    //PdfString text = (PdfString)PdfReader.GetPdfObject(mk.Get(PdfName.CA));
                    //PdfString textVal = (PdfString)PdfReader.GetPdfObject(mk.Get(PdfName.CA));
                    //BORDER COLOR
                    PdfArray carr = mk.GetAsArray(PdfName.BC);
                    if (carr != null)
                    {
                        if (carr.ArrayList.Count == 3)
                        {
                            c = new iTextSharp.text.BaseColor((int)(carr.GetAsNumber(0).DoubleValue * 255), (int)(carr.GetAsNumber(1).DoubleValue * 255), (int)(carr.GetAsNumber(2).DoubleValue * 255));
                            if (c != null)
                            {
                                bordercolor = c;
                            };
                    ...
                    //BORDERWIDTH
                    PdfNumber w = mk.GetAsNumber(PdfName.W);
                    if (w != null)
                    {
                        tx.BorderWidth = w.IntValue;
                    };
                    //W
                    //BACKGROUND COLOR
                    carr = mk.GetAsArray(PdfName.BG);
                    if (carr != null)
                    {
                        if (carr.ArrayList.Count == 3)
                        {
        ...
        public TextField GetNewComboFromField(String fieldName, int order, PdfWriter writer)
        {
            try
            {
                if (GetFieldType(fieldName) != FIELD_TYPE_COMBO)
                    return null;
                //if (order){order=0;}
                if (order <= 0)
                {
        ...
        public TextField GetNewListboxFromField(String fieldName, int order, PdfWriter writer)
        {
            try
            {
                if (GetFieldType(fieldName) != FIELD_TYPE_LIST)
                    return null;
                //if (order){order=0;}
                if (order <= 0)
                {
                ...
                if (mk != null)
                {
                    ...
                    if (aa != null)
                    {
                        tx.GetListField().Put(PdfName.AA, aa);
                        PdfDictionary k = aa.GetAsDict(PdfName.K);
                        if (k != null)
                        {
                            PdfString JsAA = k.GetAsString(PdfName.JS);
                            if (JsAA != null)
                            {
                                tx.GetListField().SetAdditionalActions(PdfAnnotation.AA_JS_KEY, PdfAction.JavaScript(JsAA.ToUnicodeString() + "", writer));
                            }
                        }
                    }
                    //W
        ...
        public RadioCheckField GetNewRadioCheckFieldFromField(String fieldName, int order, PdfWriter writer)
        {
            try
            {
                if (GetFieldType(fieldName) == FIELD_TYPE_CHECKBOX | GetFieldType(fieldName) == FIELD_TYPE_RADIOBUTTON)
                {
                    //do nothing
                }
                else
                ...
                topFirst = 0;
                RadioCheckField fld = null;
                Item item = GetFieldItem(fieldName);
                PdfDictionary merged = (PdfDictionary)item.merged[order];
                //if (fieldCache == null || !fieldCache.ContainsKey(fieldName))
                {

                    //fld = new TextField(writer, null,null);
                    IList<FieldPosition> pos = GetFieldPositions(fieldName);
                    Rectangle box = new Rectangle(pos[0].position);
                    fld = new RadioCheckField(writer, box, fieldName, "");
                    //fld.SetExtraMargin(extraMarginLeft, extraMarginTop);
                    fld.BorderWidth = 0;
                    //fld.SubstitutionFonts = substitutionFonts;
                    DecodeGenericDictionary(merged, fld);
                    //rect
                    //PdfArray rect = (PdfArray)PdfReader.GetPdfObject(merged.Get(PdfName.RECT));
                    //Rectangle box = PdfReader.GetNormalizedRectangle(rect);
                    if (fld.Rotation == 90 || fld.Rotation == 270)
                        box = box.Rotate();
                    fld.Box = box;
                    //if (fieldCache != null)
                    //    fieldCache[fieldName] = fld;
                }
                //else
                //{
                //    fld = (RadioCheckField)fieldCache[fieldName];
                //    fld.Writer = writer;
                //}
                ...
                try
                {
                    if (GetFieldType(fieldName) == FIELD_TYPE_CHECKBOX && merged.GetAsName(PdfName.AS) != null)
                    {
                        fld.OnValue = merged.GetAsName(PdfName.AS).ToString().TrimStart("/".ToCharArray()).ToString();
                        if (merged.GetAsName(PdfName.V) != null)
                        {
                            if (merged.GetAsName(PdfName.V).ToString().TrimStart("/".ToCharArray()).ToString() == merged.GetAsName(PdfName.AS).ToString().TrimStart("/".ToCharArray()).ToString())
                            {
                                fld.Checked = true;
                            }
                            else
                    ...
                    {
                        if (merged.GetAsDict(PdfName.AP).GetAsDict(PdfName.D) != null)
                        {
                            PdfName[] keys = new PdfName[merged.GetAsDict(PdfName.AP).GetAsDict(PdfName.D).Keys.Count];
                            merged.GetAsDict(PdfName.AP).GetAsDict(PdfName.D).Keys.CopyTo(keys, 0);
                            for (Int32 keyIdx = 0; keyIdx <= keys.Length - 1; keyIdx++)
                            {
                                if (keys[keyIdx].ToString().TrimStart("/".ToCharArray()).ToString().ToLower() != "Off".ToLower())
                                {
                                    fld.OnValue = keys[keyIdx].ToString().TrimStart("/".ToCharArray()).ToString();
                                    break;
                                }
                            }
                        }
                        else if (merged.GetAsDict(PdfName.AP).GetAsDict(PdfName.N) != null)
                        {
                            PdfName[] keys = new PdfName[merged.GetAsDict(PdfName.AP).GetAsDict(PdfName.N).Keys.Count];
                            merged.GetAsDict(PdfName.AP).GetAsDict(PdfName.N).Keys.CopyTo(keys, 0);
                            for (Int32 keyIdx = 0; keyIdx <= keys.Length - 1; keyIdx++)
                            {
                                if (keys[keyIdx].ToString().TrimStart("/".ToCharArray()).ToString().ToLower() != "Off".ToLower())
                                {
                ...
                try
                {
                    ...
                    //object[] dao = AcroFields.SplitDAelements(da.ToUnicodeString());
                    //string asd1 = item.GetWidget(0).GetAsString(PdfName.DA).ToString();
                    //object[] colors = AcroFields.SplitDAelements(asd1.ToString());
                    //textcolor
                    //textcolor = (Color)(colors[colors.Length - 1]);
                    //}
                }
                catch (Exception ex) { }
                ...
                if (textVal != null)
                {
                    fld.Text = textVal;//.ToUnicodeString();
                }
                if (mk != null)
                {
                    ...
                    fld.TextColor = textcolor;
                    fld.BackgroundColor = bgcolor;
                    fld.BorderColor = bordercolor;
                };
